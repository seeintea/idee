---
title: 现代 React SPA 路由实践：从 TanStack Router 到 React Router v7
date: 2026-04-17
description: 在真实 B 端项目中比较两套路由方案，并用文件约定与构建插件改善协作体验
tags: ["React", "路由", "架构", "AI 撰写"]
---
...
使用 `react-router@7` 的库模式作为稳定基座，参考文件路由的组织方式，将原先聚合的 `RouteObject` 拆散；再开发自定义 Rsbuild 插件，监听路由目录并生成完整的路由树与业务菜单。
...
我在 GitHub 上找到了类似问题的 issue，因为时间紧迫，我让 AI 协助分析了原因，反馈说 TanStack Router 团队最近重构的异步加载模块导致的（因为时间紧迫，我并没有仔细验证）。
