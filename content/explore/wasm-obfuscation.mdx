---
title: 用 Wasm 构建加解密「黑盒」的探索
date: 2026-01-17
description: 将加解密逻辑迁移到 Wasm，探索提高前端密钥提取门槛的可行性
tags: ["技术", "AI 润色"]
---
...
最近在定位线上问题时，我在打包产物里发现了一段连续字符。全局搜索后，确认它是用于数据加解密的密钥。前端代码天然透明，把密钥硬编码在代码里显然不合适。常见的补救手段，是对加解密逻辑做混淆，以提高逆向成本。
...
一个更现实的方向是：把关键逻辑迁移到 WebAssembly 中，获得一种“近似黑盒”的效果。用 Go、Rust 等语言重写加解密逻辑，编译为 Wasm 文件后再由前端调用。无论是分析打包后的 JS，还是通过调试直接定位密钥，这种方式都能增加一些阻力。
...
- 对外仅暴露 `encrypt` / `decrypt` 两个函数（使用字符串接口，尽量降低 JS 与 Wasm 的交互复杂度）

引入 Wasm 后，简单验证如下：

<AesPerformance />

将编译后的 Wasm 转换为 WAT 格式后，可以尝试直接在文本中检索密钥。转换后的代码呈现出一种类似 `Lisp` 的结构化风格：符号密集、片段离散，想从中快速定位“散落的密钥”会困难许多。
...
需要强调的是：把密钥“藏进 Wasm”并不意味着密钥真正安全。只要攻击者能在前端调用 `encrypt` / `decrypt`，就仍然可以把它当作一个加解密 oracle 使用。它的实际价值更多是“提高提取密钥的门槛”，而非“阻止功能被滥用”。要从根本上解决密钥暴露问题，仍然需要服务端参与，例如不向前端下发密钥、按会话派生密钥或对请求进行签名。
