...
import { AesPerformance } from "@/component/aes-performance";
...
<img
  className="rounded"
  src="//seeintea.github.io/static/images/explore/wasm-obfuscation-001.webp"
  alt="Wasm 加解密黑盒"
/>
...
pub fn decrypt(ciphertext_hex: &str) -> Result<String, JsValue> {
    let ct =
        key_iv_codec::hex_decode(ciphertext_hex).map_err(|e| JsValue::from_str(&e.to_string()))?;
